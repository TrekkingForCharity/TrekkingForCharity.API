...
{
    public static class DeleteWaypointCommandExecutor
    {
        [FunctionName("DeleteWaypointCommandExecutor")]
        public static async Task<HttpResponseMessage> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "commands/delete-waypoint")]
            HttpRequest req,
            ...
            ExecutionContext context)
        {
            try
            {
                ...
                var cmd = JsonConvert.DeserializeObject<DeleteWaypointCommand>(jsonContent);

                var validator = new DeleteWaypointCommandValidator();
                ...
                if (result.IsFailure)
                {
                    return HttpRequestMessageHelpers.CreateApiErrorResponseWithSingleValidationError("WaypointId", ErrorCodes.TrekNotFound,
                        $"Waypoint with Id {cmd.WaypointId} not found");
                }
                ...
                if (updateResult.IsFailure)
                {
                    return HttpRequestMessageHelpers.CreateApiErrorResponse(
                        ErrorCodes.Creation, "Something went wrong when trying to delete the waypointtrek");
                }
