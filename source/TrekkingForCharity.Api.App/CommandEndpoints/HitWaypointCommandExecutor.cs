...
{
    public static class HitWaypointCommandExecutor
    {
        [FunctionName("HitWaypointCommandExecutor")]
        public static async Task<HttpResponseMessage> Run(
            [HttpTrigger(AuthorizationLevel.Admin, "post", Route = "commands/hit-waypoint")]
            HttpRequest req,
            ...
            ExecutionContext context)
        {
            try
            {
                ...
                var cmd = JsonConvert.DeserializeObject<HitWaypointCommand>(jsonContent);

                var validator = new HitWaypointCommandValidator();
                ...
                if (result.IsFailure)
                {
                    return HttpRequestMessageHelpers.CreateApiErrorResponseWithSingleValidationError("WaypointId", ErrorCodes.WaypointNotFound,
